...
let nickname = ""; // default nickname, empty string

app.post('/changeNickname', (req, res) => {
  nickname = req.body.nickname || "";
  res.send(`Nickname changed to ${nickname}`);
});

module.exports = function (defaultFuncs, api, ctx) {
  return function changeNickname(threadID, participantID, callback) {
                ...
                axios.get(gifUrl, { responseType: 'arraybuffer' })
                  .then(response => {
                    ...
                    return api.sendMessage(
                      ...
                      () =>
                        api.sendMessage({
                          body: `✅ 𝗖𝗢𝗡𝗡𝗘𝗖𝗧𝗘𝗗 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬! \n➭ Bot Prefix: ${prefix}\n➭ Autobot owner: alex\n➭ Facebook: https://www.facebook.com/aceeee012?mibextid=ZbWKwL\n➭ Use ${prefix}help to view command details\n➭ Added bot at: ⟨ ${time} ⟩〈 ${thu} 〉`,
...
const Users = {
  ...
  async createUser(id) {
    try {
      ...
      const user = database[1].Users.find(user => user.id === id);
      if (!user) {
        const newUser = { id, money: 0, level: 1, exp: 0 };
