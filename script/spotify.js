...
module.exports.config = {
  ...
  version: "1.0.0",
  role: 0,
  credits: "Alex", //convert by Alex too
  description: "Search and play music from Spotify", //api by Deku
  hasPrefix: false,
  usages: "[song name]",
  cooldown: 10
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    ...
    if (data) {
      const { title, url } = data[0]; // assuming the API returns an array of results
